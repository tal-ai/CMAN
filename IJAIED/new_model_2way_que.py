...
def test_epoch(model, test_data, device):
    ...
    accuracy,precision,recall,f1, auc = get_metrics_binary(test_predict_prob,test_pred_label)
    accuracy_label = total_correct_label/n_total
    loss_per_sample = total_loss / n_total
    return accuracy_label, loss_per_sample, accuracy,precision,recall,f1, auc


def train(model, training_data, validation_data, test_data_que, test_data_dom, optimizer, device, opt, scheduler):
    '''Start training'''
    log_train_file = None
    log_valid_file = None
    ...
    for epoch_i in range(opt.epoch):
        ...
        if valid_accu >= max(valid_accus):
            print('[Info]: For question task:')
            test_accu_que, test_loss_que, accuracy_q,precision_q,recall_q,f1_q, auc_q = test_epoch(model,test_data_que,device)
            # print('[Info]: For question task New Test accuracy {}'.format(test_accu_que))
            print('[Info]: For domain task:')
            test_accu_dom, test_loss_dom, accuracy_d,precision_d,recall_d,f1_d, auc_d = test_epoch(model,test_data_dom,device)
        ...
        #             epoch = epoch_i, loss=valid_loss, accu_label=100*valid_accu
        #         ))
    return accuracy_a,precision_a,recall_a,f1_a, auc_a
...
def main():
    ...
    parser.add_argument('-log',type=str, default='./logs/new_model_2way_que-dom')
    parser.add_argument('-save_model',type=str, default='./saved_model/new_model_2way_que-dom')
    parser.add_argument('-save_mode', type=str, choices=['all','best'], default='best')

    opt = parser.parse_args()
    print(opt)

    accuracys_ans, recalls_ans, precisions_ans, f1s_ans, aucs_ans = [],[],[],[],[]
    for i in range(1, 2):
        seed_num = i*1000
        ...
        train_data_left = torch.LongTensor(torch.load('./semeval/sem_new_train_dev/2way/left_aug_train'))
        train_data_right = torch.LongTensor(torch.load('./semeval/sem_new_train_dev/2way/right_aug_train'))
        #yaoyongde dev data
        dev_data_left = torch.LongTensor(torch.load('./semeval/sem_new_train_dev/2way/left_aug_dev'))
        dev_data_right = torch.LongTensor(torch.load('./semeval/sem_new_train_dev/2way/right_aug_dev'))

        #yaoyongde label
        train_label = torch.FloatTensor(torch.load('./semeval/sem_new_train_dev/2way/train_aug_label')).long()
        test_label_answer = torch.FloatTensor(torch.load('./semeval/processed_data/2way/unseen_answers/test_label_answer')).long()
        dev_label = torch.FloatTensor(torch.load('./semeval/sem_new_train_dev/2way/dev_aug_label')).long()
        ...
        accuracy_a,precision_a,recall_a,f1_a, auc_a = train(model, train_dataloader, validation_dataloader, test_dataloader_que, test_dataloader_dom,optimizer, opt.device, opt,scheduler)
        accuracys_ans.append(accuracy_a)
        recalls_ans.append(recall_a)
        precisions_ans.append(precision_a)
        f1s_ans.append(f1_a)
        aucs_ans.append(auc_a)
    print('For answer')
    print('final average accuracy: {}, std: {}'.format(np.mean(accuracys_ans), np.std(accuracys_ans)))
    print('final average precision: {}, std: {}'.format(np.mean(precisions_ans), np.std(precisions_ans)))
    print('final average recall: {}, std: {}'.format(np.mean(recalls_ans), np.std(recalls_ans)))
    print('final average f1: {}, std: {}'.format(np.mean(f1s_ans), np.std(f1s_ans)))
    print('final average auc: {}, std: {}'.format(np.mean(aucs_ans), np.std(aucs_ans)))

...
def train(model, training_data, validation_data, test_data_ans, optimizer, device, opt, scheduler):
    '''Start training'''
    log_train_file = None
    log_valid_file = None
    ...
    for epoch_i in range(opt.epoch):
        ...
        if valid_accu >= max(valid_accus):
            print('[Info]: For answer task:')
            test_accu_ans, test_loss_ans,accuracy_both, micro_f1, micro_auc, macro_f1, macro_auc= test_epoch(model,test_data_ans,device)
            print('test acc:{}, micro f1:{}, micro auc:{}, macro f1:{}, macro auc:{}'.format(accuracy_both, micro_f1, micro_auc, macro_f1, macro_auc))
        ...
        #             epoch = epoch_i, loss=valid_loss, accu_label=100*valid_accu
        #         ))
    return accuracy_both, micro_f1, micro_auc, macro_f1, macro_auc
...
def main():
    ...
    parser.add_argument('-log',type=str, default='./logs/new_model_3way_ans')
    parser.add_argument('-save_model',type=str, default='./saved_model/new_model_3way_ans')
    parser.add_argument('-save_mode', type=str, choices=['all','best'], default='best')

    opt = parser.parse_args()
    print(opt)

    accuracys_ans, f1s_ans_micro, aucs_ans_mirco, f1s_ans_macro, aucs_ans_macro = [],[],[],[],[]
    for i in range(1, 2):
        seed_num = i*1000
        ...
        train_data_left = torch.LongTensor(torch.load('./semeval/sem_new_train_dev/3way/left_train'))
        train_data_right = torch.LongTensor(torch.load('./semeval/sem_new_train_dev/3way/right_train'))
        #yaoyongde dev data
        dev_data_left = torch.LongTensor(torch.load('./semeval/sem_new_train_dev/3way/left_dev'))
        dev_data_right = torch.LongTensor(torch.load('./semeval/sem_new_train_dev/3way/right_dev'))

        # yaoyongde label
        train_label = torch.LongTensor([i-1 for i in torch.load('./semeval/sem_new_train_dev/3way/train_label')])
        dev_label = torch.LongTensor([i-1 for i in torch.load('./semeval/sem_new_train_dev/3way/dev_label')])
        ...
        accuracy_both, micro_f1, micro_auc, macro_f1, macro_auc = train(model, train_dataloader, validation_dataloader, test_dataloader_ans,optimizer, opt.device, opt,scheduler)
        
        accuracys_ans.append(accuracy_both)
        f1s_ans_micro.append(micro_f1)
        aucs_ans_mirco.append(micro_auc)
        f1s_ans_macro.append(macro_f1)
        aucs_ans_macro.append(macro_auc)
    print(accuracy_both)
    print('For answer')
    print('final average accuracy: {}, std: {}'.format(np.mean(accuracys_ans), np.std(accuracys_ans)))
    print('[Micro] final average f1: {}, std: {}'.format(np.mean(f1s_ans_micro), np.std(f1s_ans_micro)))
    print('[Micro] final average auc: {}, std: {}'.format(np.mean(aucs_ans_mirco), np.std(aucs_ans_mirco)))
    print('[Macro] final average f1: {}, std: {}'.format(np.mean(f1s_ans_macro), np.std(f1s_ans_macro)))
    print('[Macro] final average auc: {}, std: {}'.format(np.mean(aucs_ans_macro), np.std(aucs_ans_macro)))

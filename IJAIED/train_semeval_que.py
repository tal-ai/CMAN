...
def test_epoch(model, test_data, device):
    ...
    # test_predict_prob = torch.cat(test_predict_prob)
...
def train(model, training_data, validation_data, test_data_que, test_data_dom,optimizer, device, opt):
    '''Start training'''
    log_train_file = None
    log_valid_file = None
    ...
    for epoch_i in range(opt.epoch):
        ...
        if valid_accu >= max(valid_accus):
                ...
                print('For question task')
                test_accu_que, test_loss_que = test_epoch(model,test_data_que,device)
                print('For domain task')
                test_accu_dom, test_loss_dom = test_epoch(model,test_data_dom,device)
...
def main():
    ...
    parser.add_argument('-log',type=str, default='./logs/2way_sem_que')
    parser.add_argument('-save_model',type=str, default='./saved_model/2way_sem_que')
    parser.add_argument('-save_mode', type=str, choices=['all','best'], default='best')

    opt = parser.parse_args()
    print(opt)
    opt.log = opt.log + '_d_model{}'.format(opt.d_model) + '_d_ff'.format(opt.d_ff) + '_numblock{}'.format(opt.number_block)
    ...
    test_data_que = TensorDataset(test_left_question, test_right_question, test_label_question)
    test_sampler_que = SequentialSampler(test_data_que)
    test_dataloader_que = DataLoader(test_data_que, sampler=test_sampler_que, batch_size=batch_size)

    # Create the DataLoader for our question test set.
    test_data_dom = TensorDataset(test_left_dom, test_right_dom, test_label_domain)
    test_sampler_dom = SequentialSampler(test_data_dom)
    test_dataloader_dom = DataLoader(test_data_dom, sampler=test_sampler_dom, batch_size=batch_size)
    ...
    train(model, train_dataloader, validation_dataloader, test_dataloader_que, test_dataloader_dom, optimizer, opt.device, opt)
